...
- **Most likely to drift:** the 3–7 KPI ceiling and the "decomposition must be mathematical" rule. Both erode under user pressure — pad-the-list pressure on the ceiling, and "drivers as decomposition" pressure on the arithmetic rule. Both are headline disciplines; reinforce inside Constraints rather than adding new sections.
- **Coupled sections:** the framework list in *Knowledge & sources* and the framework choice in Phase 2. If a framework is added or removed, both must move together. Similarly, the Spitzer criteria checklist in Phase 3 mirrors the Knowledge & sources description; keep them aligned.
- **Pairs with no knowledge file by design.** All five frameworks are named inline so the prompt is self-contained across platforms (Custom GPTs and Gems do not treat knowledge as ambient). If the framework set grows, prefer adding inline rather than externalizing — the upstream-measurement-design role does not need a heavy reference document the way `dashboard-designer.md` does.
- **Regression tests:** a request that names no scope of accountability (verify Phase 1 stops before advancing), a request listing 12 KPIs (verify the prompt cuts to 3–7), a request to "decompose customer happiness into drivers" with conceptually-related but non-arithmetic factors (verify the prompt explains correlate-vs-decomposition and reworks), a request to use lines-of-code as a productivity KPI (verify Grove's pairing rule fires with the gaming pattern named), a request to design a dashboard (verify the prompt redirects to `dashboard-designer.md`).
- **Do not flatten:** the five-phase conversation flow, the Spitzer checklist, or the requirement that decomposition reconstruct the KPI arithmetically. The DuPont mechanic is the load-bearing structural idea — it is what differentiates a KPI tree from a brainstorm.
