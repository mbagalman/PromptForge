...
- **Most likely to drift:** the 3–7 KPI ceiling, the Mode A vs. Mode B distinction, and the priority order. The ceiling is mitigated by the *Non-KPI metrics* section but still erodes under pad-the-list pressure; the mode distinction erodes under "force a multiplicative identity on a survey-based KPI" pressure; the priority order erodes when a downstream constraint (output formatting) tries to win over an upstream one (scope, decision-usefulness). All three are headline disciplines; reinforce inside Constraints or the Priority-order block rather than adding new sections.
- **Coupled sections:** the framework list in *Knowledge & sources* and the framework choice in Phase 2. The Spitzer criteria checklist in Phase 3 mirrors the Knowledge & sources description; keep them aligned. The Metric taxonomy block (KPI / counter-metric / driver / diagnostic / monitoring / candidate — six entries) is referenced throughout the spec, the *Non-KPI metrics* Role column, and the conflict-resolution priority order; if the taxonomy changes, audit every cross-reference. The per-KPI fields in the Output contract template (formula / scoring method, unit, inclusion/exclusion, data source, refresh cadence, counter-metric rationale, known caveats) are the definition-hygiene contract; if a downstream consumer needs new fields (e.g., regulatory ID, system-of-record version), add them here.
...
- **Regression tests:** a request that names no scope of accountability (verify Phase 1 stops with a scope-clarification close, not a fabricated Phase 5 spec, per priority rule 1); a request listing 12 KPIs (verify the prompt cuts to 3–7 and files the overflow under *Non-KPI metrics* with role classifications, or marks the spec "expanded KPI set" if the user explicitly overrides); a request to "decompose customer happiness into drivers" (verify the prompt switches to Mode B measurement model and labels it honestly); a request to build a Mode A decomposition for NPS or employee engagement (verify the prompt refuses to invent a multiplicative identity, switches to Mode B, and the *Formula / scoring method* field captures the scoring method rather than a fake arithmetic formula); a request to use lines-of-code as a productivity KPI (verify Grove's pairing rule fires with the gaming pattern named, and the *Counter-metric rationale* field is populated); a healthcare-quality request (verify Donabedian or Triple Aim is invoked by name); a complex-organization request with 7 mixed Mode A and Mode B KPIs (verify Phase 4 extends past 2 exchanges when warranted); a request to design a dashboard (verify the prompt redirects to `dashboard-designer.md`).
- **Do not flatten:** the five-phase conversation flow, the Spitzer checklist, the Metric taxonomy block, the Mode A / Mode B distinction, or the Priority-order block. The DuPont mechanic (Mode A) is what differentiates a KPI tree from a brainstorm for quantitative outcome KPIs; the measurement model (Mode B) is what keeps the prompt honest about KPIs that have no arithmetic identity; the Priority-order block is what keeps the assistant from sacrificing scope or decision-usefulness for output-template fidelity in edge cases.
