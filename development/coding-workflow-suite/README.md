...
The suite is distinct from — and complementary to — the [`coding/`](../../coding/README.md) directory, which ships per-language code-quality tools (`python-hardener.md`, `sql-optimization-engineer.md`) that operate during or after implementation. The workflow suite operates *before* code is written (planning, design, decisions); the `coding/` tools operate *during* or *after* (hardening Python, optimizing SQL). The two folders are independent and either can be used without the other; together they cover the planning-through-implementation arc. The suite is also distinct from `guides/agents-md-best-practices-2026.md`, which is reference material for writing AGENTS.md files; `agents-md-generator.md` is the prompt that applies that guidance to a project's Implementation Plan and ADRs.
